...
class ActionButton extends React.Component {
  ...
  transformAclToActions(acl) {
    return _(acl)
      ...
      .keys()
      // ['send_application']
      .map(acl => ({id: acl, label: this.aclToLabel[acl], trigger: this.props.handleSubmitFactory(acl)}))
  ...
  render() {
    ...
    return (
      actions.map(({id, label, trigger}) => (
        <button key={id} className='button' onClick={trigger}>{label}</button>
      ))
    )
  }
}
