...
describe('demarches', () => {
  ...
  Object.entries(allDemarches).forEach(apiDemarcheskVp => {
    ...
    it(`default demarche' state should contain every fields that appear in other demarches ${
      apiDemarcheskVp[0]
    }`, () => {
      ...
      Object.entries(apiDemarcheskVp[1]).forEach(keyValue => {
        Object.keys(keyValue[1].state).forEach(field => {
          if (defaultDemarche.state[field] === undefined) {
            console.log(
              `field ${field} not present in default demarche of ${
                apiDemarcheskVp[0]
              }`
            );
            everyFields = false;
  ...
  Object.entries(allDemarches).forEach(apiDemarcheskVp => {
    ...
    it(`default demarche' scope should contain every scope's field that appear in other demarches ${
      apiDemarcheskVp[0]
    }`, () => {
      ...
      Object.entries(apiDemarcheskVp[1]).forEach(keyValue => {
        Object.keys(keyValue[1].state.scopes || []).forEach(field => {
          if (defaultDemarche.state.scopes[field] === undefined) {
            console.log(
              `field ${field} not present in default demarche's scope of ${
                apiDemarcheskVp[0]
              }`
            );
