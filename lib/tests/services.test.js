...
describe('createUserEnrollment', () => {
  describe('When the user is not authorized', () => {
    ...
    it('should return a 401', () => {
      ...
      const token = 'boom'
      return Services.createUserEnrollment(state, token).catch(error => {
        expect(error.response.status).toBe(401)
      })
    })
  })
  describe('When the user is authorized', () => {
    describe('When the payload is no empty', () => {
      nock(BACK_HOST, {
        reqheaders: {
...
describe('updateUserEnrollment', () => {
  describe('When there is a response', () => {
    ...
    }).patch('/api/enrollments/1', payload)
      .reply(200, VALIDATED_ENROLLMENT)
    it('should return the data', () => {
      const token = 'test'
      const id = 1
      return Services.updateUserEnrollment({enrollment: PENDING_ENROLLMENT}, token, id).then(response => {
