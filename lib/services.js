...
const Services = {
  ...
  createUserEnrollment({enrollment}, token) {
    ...
    const formData = this.serializeEnrollment({enrollment})
    return axios.post(BACK_HOST + '/api/enrollments/', formData.toString(), {
      headers: {
  ...
  updateUserEnrollment({enrollment}, token) {
    token = token || localStorage.getItem('token')
    const formData = this.serializeEnrollment({enrollment})
    return axios.patch(BACK_HOST + '/api/enrollments/' + enrollment.id, formData.toString(), {
      headers: {
        Authorization: 'Bearer ' + token,
        'Content-Type': 'multipart/form-data'
      }
